...
function StockView() {
	...
	useEffect(() => {
		...
		if (tokens.isAuthenticated()) {
			const currentUsername = tokens.getUsername(); // Récupération du username
			console.log("Username utilisé pour watchlist:", currentUsername); // Log de vérification
			
			accounts.getWatchlist(currentUsername, true).then((res: any[]) => {
	...
	const handleWatchlistAction = async (action: "add" | "remove") => {
		try {
			const currentUsername = tokens.getUsername();
			console.log(`Watchlist ${action} for user:`, currentUsername); // Log de vérification
			
			await accounts.editWatchlist(currentUsername, symbol as string, action);
			setOnWatchlist(action === "add");
		} catch (error) {
			console.error(`Failed to ${action} to watchlist:`, error);
		}
	};
